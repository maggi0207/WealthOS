...
/** Quick actions for the goals module. */
export function GoalQuickActions() {
  ...
  return (
    <>
        ...
        {goalQuickActions.map((action) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                if (action.id === "new-goal") setGoalFormOpen(true);
                else if (action.id === "add-funds") setContribOpen(true);
                else toast.info(`${action.label} — coming soon`);
              }}
      ...
      <GoalFormSheet open={goalFormOpen} onOpenChange={setGoalFormOpen} mode="create" />
      <ContributionFormSheet open={contribOpen} onOpenChange={setContribOpen} />
    </>
  );
}
