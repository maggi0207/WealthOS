...
import { allocation, allocationTotal, fmtINRShort } from "@/lib/wealth-data";
import { cn } from "@/lib/utils";

/** Animated allocation donut with a tappable legend that highlights a slice. */
export function AllocationDonut() {
  ...
  const centreLabel = focus ? focus.name : "Total assets";
  const centreValue = focus ? fmtINRShort(focus.value) : fmtINRShort(allocationTotal);
  ...
  return (
    <section className="surface-tile p-4">
      <div className="relative">
        ...
        <div className="pointer-events-none absolute inset-0 grid place-items-center">
          <div className="text-center">
            ...
            <p className="mt-0.5 font-display text-xl font-semibold tabular-nums sm:text-2xl">{centreValue}</p>
          </div>
        </div>
      </div>
      ...
      <ul className="mt-3 grid gap-1.5">
        {allocation.map((slice, index) => {
          const pct = (slice.value / allocationTotal) * 100;
          return (
            <li key={slice.name}>
              <button
                type="button"
